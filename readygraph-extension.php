<?php
  // Extension Configuration
	$ee_plugin_slug = basename(dirname(__FILE__));
	$ee_menu_slug = 'readygraph-app';
	$ee_main_plugin_title = 'Email Newsletter';

	add_action( 'wp_ajax_nopriv_ee-myajax-submit', 'ee_myajax_submit' );
	add_action( 'wp_ajax_ee-myajax-submit', 'ee_myajax_submit' );
...
function ee_myajax_submit() {
  ...
  {
    global $ee_plugin_slug, $ee_menu_slug;
    append_submenu_page($ee_plugin_slug, 'Readygraph App', __( 'Readygraph App', $ee_plugin_slug), 'administrator', $ee_menu_slug, 'add_readygraph_page');
  }
...
function on_plugin_activated_readygraph_ee_redirect(){
	global $ee_menu_slug;
	$setting_url="admin.php?page=$ee_menu_slug";    
	if (get_option('rg_ee_plugin_do_activation_redirect', false)) {  
		delete_option('rg_ee_plugin_do_activation_redirect'); 
...
function add_ee_readygraph_plugin_warning() {
  ...
  global $hook_suffix, $current_user, $ee_menu_slug;
  ...
  if ( $hook_suffix == 'plugins.php' && get_option('readygraph_connect_notice') == 'true' ) {              
    echo '<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">  
      ...
      <form name="readygraph_activate" action="'.admin_url( 'admin.php?page=' . $ee_menu_slug).'" method="POST"> 
	...
	add_action('wp_footer', 'ee_readygraph_client_script_head', 7);
	add_action('admin_init', 'on_plugin_activated_readygraph_ee_redirect');
...
function ee_post_updated_send_email( $post_id ) {
	...
	if(get_option('ee_wordpress_sync_users')) return;
	else {
		add_action('plugins_loaded', 'ee_wordpress_sync_users');
	}
function ee_wordpress_sync_users() {
	if(get_option('ee_wordpress_sync_users') && get_option('ee_wordpress_sync_users') == "true") return;
	else {
		if(get_option('readygraph_application_id') && strlen(get_option('readygraph_application_id')) > 0){
			global $wpdb;
			$app_id = get_option('readygraph_application_id');
			$query = "SELECT ee_email as email, ee_date as user_date FROM {$wpdb->prefix}ee ";
