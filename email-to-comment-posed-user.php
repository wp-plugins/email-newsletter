...
<?php
echo "<h2>"; 
echo wp_specialchars( "Email newsletter (Send Email)" ) ;
echo "</h2>";
?>
...
<form name="form_eemail" method="post" action="options-general.php?page=email-newsletter/email-to-comment-posed-user.php" onsubmit="return send_email_submit()"  >
<input type="hidden" name="send" value="true" />
<?php
global $wpdb, $wp_version;

$sSql = "SELECT DISTINCT(comment_author_email) as user_email, `comment_author`,`comment_author_email` FROM `wp_comments` WHERE 1 and comment_author_email <> ''";
